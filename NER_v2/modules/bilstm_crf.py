...
from .feature_compose import EmbedCompose, FeatureCompose
'''
...
class BiLSTMCRF(nn.Module):
    def __init__(self, args, wd_embedding_weights, signal_embedding_weights):
        super(BiLSTMCRF, self).__init__()
        self._args = args

        self._wd_embed_dim = wd_embedding_weights.shape[1]
        self._wd_embed = nn.Embedding.from_pretrained(torch.from_numpy(wd_embedding_weights))
        self._wd_embed.weight.requires_grad = False

        self._sig_dim = signal_embedding_weights.shape[1]
        self._signal_embed = nn.Embedding.from_pretrained(torch.from_numpy(signal_embedding_weights))
        ...
        self._bilstm = RNNEncoder(input_size=args.char_embed_dim,
        ...
        self._embed_compose = EmbedCompose(self._wd_embed_dim)
        self._feature_compose = FeatureCompose(args.hidden_size, self._sig_dim)
    ...
    def _get_emissions(self, wd_input, ch_input, non_pad_mask):
        ...
        # (batch_size, seq_len, signal_embed_dim)
        sig_embed = self._signal_embed(wd_input)
        # compose word embedding and char embedding
        embed = self._embed_compose(wd_embed, ch_embed, sig_embed)
        ...
        out = self._feature_compose(rnn_out, sig_embed)
        ...
        if self.training:
            out = self._drop_linear(out)

        # (batch_size, seq_len, tag_size)
        emissions = self._linear(out)  # 非归一化score
    ...
    def calc_loss(self, wd_input, ch_input, tags, non_pad_mask=None):
        ...
        if non_pad_mask is None:
            non_pad_mask = wd_input.ne(self._args.pad)  # 非运算求mask
    ...
    def forward(self, wd_input, ch_input, non_pad_mask=None):
        ...
        return torch.tensor(tags_seq, device=wd_input.device)
